...
export default function ProfitLossManagement() {
  ...
  const handleSubmit = async e => {
    ...
    if (form.date > today) { alert('Date cannot be in the future.'); return; }
    const sections = ['revenue','cogs','expenses','other'];
    for (const section of sections) {
      for (const [_, val] of Object.entries(form[section])) {
        if (val < 0) { alert('Values cannot be negative.'); return; }
      }
    }
    const payload = { ...form, date: form.date };
    if (editingId) await axios.put(`${baseUrl}/update/${editingId}`, payload);
    else await axios.post(`${baseUrl}/add`, payload);
